...
| 异步知识库导入 | 记录导入、解析、Embedding、索引任务状态，并通过消息中心反馈成功或失败 |
...
知识库管理端以“知识库”页面为主：管理知识空间、上传文档、查看切片和执行检索测试。
文档导入仍然是异步任务，后台会记录 `kb_ingest_job` 状态，并通过消息中心反馈成功或失败，
不再额外维护独立的任务中心页面。
...
| AI 问答 | 基于博客文章 + 知识库文档的 RAG 智能问答（向量检索 + 关键词 fallback + DeepSeek LLM），首页中央入口，流式对话 |
...
- 博客前台采用“AI 知识库式技术博客”布局：首页以中央 AI 问答为入口，下方承接最新文章流和说说动态。
